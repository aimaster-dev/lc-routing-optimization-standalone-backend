...
async def generate_route_map(result_dict: dict):
    API_KEY = 'xxx'
    trial = True
    dictionary_outcome = {}
    ...
    while pointer < len(containers):
        ...
        if swg[pointer] == 0:
            route.append(pointer)
            if pointer + 1 < len(containers) and swg[pointer+1] == 1:
                route.append(0)
            pointer += 1
        else:
            if pointer < len(containers) - 1:
                if swg[pointer+1] == swg[pointer] and containers[pointer+1] == containers[pointer]:
                    pointer += 1
                    continue
                elif swg[pointer+1] != swg[pointer]:
                    route.append(0)
                    pointer += 1
                    continue
                elif swg[pointer+1] == swg[pointer] and containers[pointer+1] != containers[pointer]:
                    route.append(0)
                    pointer += 1
                    continue
            else:
    ...
    for key in fullDictionary.keys():
        ...
        if len(fullDictionary[key]):
            ...
            while len(fullDictionary[key]):
                ...
                routeOptimizedNew.append(1)
                arrival_time += time_matrix[1, nextCustomer]
    ...
    # route_map_optimal.save(f"maps/optimal_map_new.html")
